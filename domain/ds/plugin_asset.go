...
type PluginAsset struct {
	...
	OwnerKey     string            `json:"ownerKey" gorm:"type:varchar(128);not null;index:idx_plugin_asset_owner;comment:钱包索引键"`          // 钱包索引键。
	...
	FactAssetId  int64             `json:"factAssetId,string" gorm:"not null;index:idx_plugin_asset_fact_asset;comment:插件资产实例ID"`          // 插件资产实例ID。
	PluginId     string            `json:"pluginId" gorm:"type:varchar(128);not null;index:idx_plugin_asset_plugin;comment:插件业务ID"`        // 插件业务ID。
	ReleaseId    int64             `json:"releaseId,string" gorm:"not null;index:idx_plugin_asset_release;comment:发布记录ID"`                 // 发布记录ID。
...
type PluginAssetBinding struct {
	...
	OwnerKey      string                   `json:"ownerKey" gorm:"type:varchar(128);not null;index:idx_plugin_asset_binding_owner;comment:钱包索引键"`           // 钱包索引键。
	FactAssetId   int64                    `json:"factAssetId,string" gorm:"not null;index:idx_plugin_asset_binding_fact_asset;comment:插件资产实例ID"`           // 插件资产实例ID。
...
type PluginInstanceState struct {
	...
	OwnerKey    string `json:"ownerKey" gorm:"type:varchar(128);not null;index:idx_plugin_instance_state_owner;comment:钱包索引键"`       // 钱包索引键。
	FactAssetId int64  `json:"factAssetId,string" gorm:"not null;uniqueIndex:idx_plugin_instance_state_fact_asset;comment:插件资产实例ID"` // 插件资产实例ID。
