...
func LikeAdd(ctx *contextx.AppContext) {
	...
	e.PanicParameterError(ctx.Gin.ShouldBindJSON(&form))
	e.PanicIfParameterError(form.Id == 0 || form.BizType == 0, "参数错误")

	e.PanicServerErr(form.Add(ctx.User))
...
func LikeDel(ctx *contextx.AppContext) {
	...
	e.PanicServerErr(form.Delete(ctx.User))
	app.Response(ctx.Gin, e.Success)
}
...
func LikeGetSummary(ctx *gin.Context) {
	...
	e.PanicParameterError(ctx.ShouldBindJSON(&param))
